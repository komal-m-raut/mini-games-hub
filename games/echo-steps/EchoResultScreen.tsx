...
import { EchoStepsResult } from './types';
...
> = {
  ...
  Great: {
    emoji: '✨',
    color: '#06B6D4',
    message: 'Sharp ears and sharper recall.',
    confetti: 'great',
  },
  Good: {
    emoji: '👍',
    color: '#A78BFA',
    message: 'Solid streak.',
    confetti: 'good',
  },
  'Try Again': {
    emoji: '🔁',
    color: '#94A3B8',
    message: 'Watch and listen a little longer next time.',
    confetti: null,
  },
};

interface EchoResultScreenProps {
  result: EchoStepsResult;
  nextLabel: string;
  onNext: () => void;
  onMenu: () => void;
}

export function EchoResultScreen({ result, nextLabel, onNext, onMenu }: EchoResultScreenProps) {
    ...
    >
      ...
      >
        ...
        <p className="font-score text-xs text-ink-3 mt-1">
          {result.len} note{result.len === 1 ? '' : 's'} recalled
        </p>
      </motion.div>
