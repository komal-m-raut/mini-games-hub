    ...
    <AnimatePresence>
      {visible && (
        ...
        >
          ...
          {isTarget && (
            <motion.div
              className="absolute -top-7 left-1/2 -translate-x-1/2 text-xs font-ui text-white/50 whitespace-nowrap px-2 py-0.5 rounded bg-white/5 border border-white/10"
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
  ...
  return (
    <div ref={containerRef} className="flex items-end justify-center gap-8 sm:gap-24 py-8">
      <div className="flex flex-col items-center gap-3">
        ...
        <p className="text-xs font-ui text-white/55 uppercase tracking-widest">Target</p>
      </div>
      <div className="flex flex-col items-center gap-3">
        <Balloon units={actualUnits} color={color} id="cmp-actual" visible scale={scale} />
        <p className="text-xs font-ui text-white/55 uppercase tracking-widest">Yours</p>
      </div>
    </div>
  );
}
