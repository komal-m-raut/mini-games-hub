...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const description =
    "You've been challenged! Sweep the same three zones as your friends and climb the shared leaderboard.";
  return {
    title: `${challengeLabel(code)} · Timing Tap`,
    description,
    // Unbounded unique URLs — keep out of search indexes, but OG stays
    // so shared invite links unfurl nicely in chats.
    robots: { index: false, follow: false },
    openGraph: { title: `${challengeLabel(code)} · Timing Tap`, description },
  };
}

export default async function TimingTapChallengePage({ params }: Props) {
  const { code } = await params;
  ...
  return (
    <div className="page-container py-8 sm:py-12 flex flex-col gap-12">
      <div>
        ...
        <TimingTapGame challengeCode={code} />
      </div>
    </div>
  );
}
