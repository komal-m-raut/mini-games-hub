...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const description =
    "You've been challenged! Name the same ink colours as your friends and climb the shared leaderboard.";
  return {
    title: `${challengeLabel(code)} · Stroop Snap`,
    description,
    // Unbounded unique URLs — keep out of search indexes, but OG stays
    // so shared invite links unfurl nicely in chats.
    robots: { index: false, follow: false },
    openGraph: { title: `${challengeLabel(code)} · Stroop Snap`, description },
  };
}

export default async function StroopSnapChallengePage({ params }: Props) {
  const { code } = await params;
  ...
  return (
    <div className="page-container py-8 sm:py-12 flex flex-col gap-12">
      <div>
        ...
        <StroopSnapGame challengeCode={code} />
      </div>
    </div>
  );
}
